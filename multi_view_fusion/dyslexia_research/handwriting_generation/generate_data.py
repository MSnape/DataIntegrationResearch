...
# Script used to create the images and text samples for the generated dataset

# Generate 500 sentences
sentences = generate_sentences(500)
...
# Save them to a file so can inspect
with open("../data/random_sentences.txt", "w") as f:
    for sentence in sentences:
        f.write(sentence + "\n")
print("\nAll 500 sentences saved to random_sentences.txt")
...
# Save them to a file so can inspect
with open("../data/random_dyslexic_sentences.txt", "w") as f:
    for sentence in dyslexic_sentences:
        f.write(sentence + "\n")
print("\nAll 500 sentences saved to random_dyselxic_sentences.txt")

# Now create the images for these sentences 
FONT_DIR = 'fonts' # Directory where handwriting font files are located
OUTPUT_NORMAL_DIR = '../data/normal_handwriting_images'
OUTPUT_DYSLEXIC_DIR = '../data/dyslexic_handwriting_images'
...
file_path = '../data/random_sentences.txt'
dyslexic_file_path = '../data/random_dyslexic_sentences.txt'
...
for i, sample_text in enumerate(text_samples):
    ...
    # Generate Normal Handwriting Image
    normal_img = generate_handwriting_image(sample_text, selected_font, 'normal')
    if normal_img:
...
# --- Configuration ---
NUM_SAMPLES = 500 # As specified, 500 samples for each category

# Base directories for generated images
NORMAL_IMG_DIR = os.path.join('..', 'data', 'normal_handwriting_images')
DYSLEXIC_IMG_DIR = os.path.join('..', 'data', 'dyslexic_handwriting_images')

CSV_OUTPUT_FILE = 'dyslexia_handwriting_dataset.csv'
...
if len(sentences) != NUM_SAMPLES or len(dyslexic_sentences) != NUM_SAMPLES:
    ...
    print(f"Please ensure your 'sentences' and 'dyslexic_sentences' lists contain {NUM_SAMPLES} entries each.")
    NUM_SAMPLES = min(len(sentences), len(dyslexic_sentences))

# --- Prepare Data for CSV ---
data = []

# Add normal handwriting samples (classification = 0)
print(f"Collecting data for {NUM_SAMPLES} normal handwriting samples...")
for i in range(NUM_SAMPLES):
    # Construct file path, ensuring consistent formatting (e.g., 'normal_sample_001.jpg')
