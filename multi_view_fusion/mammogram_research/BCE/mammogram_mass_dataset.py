...
# This is mainly a copy of the MammogramDataset
class MammogramMassDataset:
    """
    Custom dataset class for loading mammogram DICOM (.dcm) files for masses (rather than calcifications)
    from the CBIS-DDSM dataset structure, grouping images by individual breast.
    ...
    only Region of Interest (ROI) with both 'MLO' and 'CC' views are included. It also includes
    'breast density' and 'pathology' for each ROI entity, with consistency checks.
    """

    def __init__(self, root_dir, csv_path=None):
        """
        Initializes the dataset: collecting and grouping all relevant DICOM file paths
    ...
    def _load_and_filter_csv_metadata(self) -> dict[tuple[str, str, str | None], Any]:
        ...
        required_cols = ['image file path', 'breast_density', 'pathology', 'abnormality id']
        ...
        for _, row in all_csv_data.iterrows():
            current_density = row['breast_density']
            ...
            for series_folder_name_from_csv in potential_series_folder_names:
                ...
                # keys for which there may be a cropped MLO and CC view
    ...
    def _collect_files(self):
        ...
        by unique ROI entity (patient_id, breast_side, abnormality_id). Applies CSV-based filtering
        if CSV paths are provided.
        """        
        # Determine valid entities based on CSVs (if provided)
    ...
    def __getitem__(self, idx):
        ...
        for image_info in roi_data['images']:
            ...
            try:
                ...
                # Here, we're just skipping problematic individual images within a ROI entity, 
                # but might want to log more severely if we add in a logging file.
