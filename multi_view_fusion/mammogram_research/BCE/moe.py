...
class MoEViewFusion(nn.Module):
    ...
    def forward(self, x_views):
        ...
        feature_extractor = torch.nn.Sequential(*(list(self.cc_expert.children())[:-1]))
        cc_features = feature_extractor(cc_image_batch)
        ...
        feature_extractor = torch.nn.Sequential(*(list(self.mlo_expert.children())[:-1]))
        mlo_features = feature_extractor(mlo_image_batch)
